...
CMD="CARGO_TARGET_DIR=\"$TMP_DIR\" RUSTFLAGS=\"-Ctarget-cpu=native --cfg nightly\" RUSTDOCFLAGS=\"-Ctarget-cpu=native --cfg nightly -Z unstable-options --generate-link-to-definition --html-in-header ./config/rustdoc-header.html\" cargo +nightly doc --no-deps -F _docsrs"
